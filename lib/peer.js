...
class PeerConnection extends EventTarget {
    ...
    broadcastMessage(message) {
        let conns = Object.entries(this.connections);
        // TODO: handle multiple people
        console.log(this.connections);
        assert(conns.length === 1, "TODO: handle multiple people");
        let [ [ , { conn } ] ] = conns;
        // console.log(conns[0], conns[0]?.conn, conns[0]?.conn?.send);
        conn.send({
            type: "chatMessage",
            content: message,
        });
    ...
    handleData(id, data) {
        ...
        // guest packets
            ...
            let { conn, reject } = this.outgoingConnections[id];
            conn.close();
            delete this.outgoingConnections[id];
            ...
            this.dispatchEvent(new CustomEvent("chatMessage", { detail: {
                ...
                from: conn.metadata.name, // TODO: broker individual metadata rather than using the connection's metadata
            } }));
        }
    }
}
